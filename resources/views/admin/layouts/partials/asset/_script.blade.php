        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.fetchFiles = async function fetchFiles(page = 1) {
                const response = await fetch(
                    `{{ route('uploads.list') }}?page=${page}&search=${encodeURIComponent(searchQuery)}`
                );
...
@endisset
{{-- @isset($slug) //with bangla support
    <script>
        function generateSlug(input, output) {
            let title = input.value;
            let isBangla = /[\u0980-\u09FF]/.test(title);

            let slug = title.toLowerCase()
                .replace(isBangla ? /[^\u0980-\u09FFa-z0-9\s-]/g : /[^a-z0-9\s-]/g, '')
                .replace(/\s+/g, '-')
                .replace(/-+/g, '-');
            output.value = slug;
        }
